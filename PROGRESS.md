...
| カラム | 正しい形式 | NG例 |
|--------|-----------|------|
| `remaining` | `'70%'`（整数%付き文字列） | `'0.7'`, `70`, `0.7` |
| `opened_date` | `'2026-03-13'`（YYYY-MM-DD） | ISO文字列 |
| `name` | `brand + number` 連結（スペースなし）例: `'キンミヤ22'` | `'キンミヤ 22'` |

> **理由**: `BottleCard.remainingPercent()` が `parseInt(remaining)` でバーを描画。`parseInt('0.7')` = 0 → スライダーが0%になる。
